...
def test_safe_contract_probe_matches_fresh_full_scan(tmp_path):
    executable = _executable(_binary())
    probed = probe_scanner_contract(
        executable=executable,
        drive_root=tmp_path,
        scope="session",
        owner_task_id="",
    )
    scanned = scan_named_bytes(
        {"safe.txt": b"ordinary fixture text\n"},
        executable=executable,
        drive_root=tmp_path,
        scope="session",
        owner_task_id="",
    ...
    assert probed.version == scanned.version == "1.8.1"
    assert probed.ruleset_sha256 == scanned.ruleset_sha256
    assert probed.scan_contract_sha256 == scanned.scan_contract_sha256

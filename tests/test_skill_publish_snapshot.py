...
def test_snapshot_hash_parity_and_full_public_control_views(tmp_path):
    ...
    (skill_dir / ".pytest_cache" / "v" / "cache" / "nodeids").write_bytes(b"cache churn")
...
def test_snapshot_rejects_review_mismatch_without_live_rehash(tmp_path):
    ...
    candidate = capture_skill_publish_candidate(loaded)
    assert candidate.content_hash == compute_content_hash(skill_dir)
    assert candidate.content_hash != loaded.review.content_hash

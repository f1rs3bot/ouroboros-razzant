...
@pytest.mark.parametrize("reason_code", ["scanner_missing", "scanner_corrupt"])
def test_runtime_repair_hints_use_the_one_exact_installer_command(reason_code):
    ...
    assert result.repair_hint.startswith("Run `")
...
def test_non_runtime_repair_hints_keep_installer_conditional(reason_code):
    result = scanner._failure(reason_code)

    assert result.repair_hint.startswith("Inspect the Betterleaks failure and retry.")
    assert betterleaks_runtime.BETTERLEAKS_INSTALL_COMMAND in result.repair_hint
    assert "only if a later preflight reports a missing or corrupt scanner runtime" in (result.repair_hint)

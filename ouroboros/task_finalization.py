...
def quarantine_model_output(
    preserve_root: Any,
    task_id: Any,
    text: Any,
    usage: Dict[str, Any],
) -> str:
    """Withhold one structurally invalid model output and retain exact raw bytes."""
    raw = str(text or "")
...
def quarantine_terminal_model_output(
    ...
    """Defend terminal model-origin projections without rewriting host-authored text."""
    if str(usage.get("terminal_origin") or "") != TERMINAL_ORIGIN_MODEL_FINAL:
        return str(text or "")
    return quarantine_model_output(
        task.get("budget_drive_root") or preserve_root, task.get("id"), text, usage,
    )

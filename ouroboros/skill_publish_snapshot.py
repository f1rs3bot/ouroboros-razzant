...
def capture_skill_publish_candidate(loaded: LoadedSkill) -> SkillPublishSnapshot:
    """Capture the exact current bytes without consulting review authority.
    ...
    once. Selected preflight uses this candidate even when review is pending or
    stale; the authoritative publication wrapper below adds the review binding.
    """
    try:

...
ScannerContractStatus = Literal["ready", "scanner_error"]
...
@dataclass(frozen=True)
class ScannerContractResult:
    """Safe current scanner identity facts, never the ruleset body."""

    status: ScannerContractStatus
    engine: str
    version: str
    ruleset_sha256: str
    scan_contract_sha256: str
...
def _repair_hint(reason_code: str) -> str:
    ...
    if reason_code in {"scanner_missing", "scanner_corrupt"}:
        return f"Run `{BETTERLEAKS_INSTALL_COMMAND}`, then retry."
    return (
        "Inspect the Betterleaks failure and retry. "
        f"Run `{BETTERLEAKS_INSTALL_COMMAND}` only if a later preflight reports "
        "a missing or corrupt scanner runtime."
    )


def _scanner_executable_reason(executable: ScannerExecutable) -> str:
    if executable.status == "missing":
        return "scanner_missing"
    if executable.status != "ready":
        return "scanner_corrupt"
...
def _contract_failure(reason_code: str) -> ScannerContractResult:
    safe_reason = reason_code if reason_code in VALID_SCANNER_REASON_CODES else "scanner_report_invalid"
    return ScannerContractResult(
        ...
        repair_hint=_repair_hint(safe_reason),
    )
...
def _prepare_host_control_files(root: pathlib.Path) -> tuple[pathlib.Path, pathlib.Path]:
...
def probe_scanner_contract(
    *,
    ...
    """Probe only safe current contract facts for exact cache lookup.

    A later cache miss deliberately calls ``scan_named_bytes`` normally, which
    repeats this probe rather than trusting a caller-provided contract token.
    """
    if scope not in {"task", "session"} or not isinstance(timeout_sec, int) or timeout_sec <= 0:
        return _contract_failure("scanner_input_invalid")
    executable_reason = _scanner_executable_reason(executable)
    if executable_reason:
        return _contract_failure(executable_reason)
    try:
        with tempfile.TemporaryDirectory(prefix="ouroboros-skill-publish-contract-") as tmp:
            root = pathlib.Path(tmp)
            try:
                os.chmod(root, 0o700)
            except OSError:
                pass
            config_path, _ignore_path = _prepare_host_control_files(root)
            ...
            if contract is None:
                return _contract_failure(reason)
            return ScannerContractResult(
                status="ready",
                engine=BETTERLEAKS_ENGINE,
        ...
        return _contract_failure("scanner_report_invalid")
...
def scan_named_bytes(
    ...
    *,
    ...
    executable_reason = _scanner_executable_reason(executable)
    if executable_reason:
        return _failure(executable_reason)
    assert executable.path is not None
    binary = pathlib.Path(executable.path)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="ouroboros-skill-publish-scan-") as tmp:
            ...
            try:
                ...
                pass
            config_path, ignore_path = _prepare_host_control_files(root)

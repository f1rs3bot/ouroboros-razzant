...
[![Version 7.0.5](https://img.shields.io/badge/version-7.0.5-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v7.0.5/Ouroboros-7.0.5.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v7.0.5/Ouroboros-7.0.5-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v7.0.5/ouroboros_7.0.5_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.5/ouroboros-7.0.5-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.5/ouroboros-7.0.5-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.5/Ouroboros-7.0.5-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.5/Ouroboros-7.0.5-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 7.0.5 | 2026-09-12 | **feat: the Safety Supervisor walks the owner's fallback chain, and a provider's content-policy refusal is a typed permanent class.** Two owner incidents shared one root: the safety lane had exactly one model and no rotation — a 429 on the Light route stalled every guarded call for ~1.5h, and a 502 whose body carried OpenAI's `cyber_policy` code (returned via a codex-lb proxy for a non-security task) blocked guarded calls with a false `SAFETY_VIOLATION` verdict. The safety check now walks `[light] + OUROBOROS_MODEL_FALLBACKS`: a genuine throughput 429 keeps its bounded retry per candidate, a structured content-policy refusal rotates without a paid same-model retry (it is permanent for that request), quota/auth/permanent classes never rotate, and an all-refused walk ends in the honest `SAFETY_UNAVAILABLE` infrastructure outcome, never a verdict. In the main loop, structured policy codes (`cyber_policy`, `content_filter`, `guardrails`, …) classify as the permanent `provider_policy_refusal` — so the request rotates to the owner's fallback chain immediately instead of burning the transient-retry budget — and codex-lb's `string_above_max_length` on `instructions` classifies as context overflow, so the compaction ladder shrinks the prompt instead of failing the route. |
| 7.0.4 | 2026-09-12 | **fix: the size-ratchet debt authority is recorded, not inherited.** v7.0.3 shrank `server.py` from 1628 to 1593 lines and therefore left `GIANT_PATHS`, but its regenerated manifest could not be committed: the manifest is a frozen-contract path and that cycle ran in `advanced` mode, where the commit gate unstages a protected path. The residue shipped — the official blocking `size_ratchet` lane was red on the committed tip, and the same unwritable artifact had forced that cycle's preflight to run with tests skipped. This regenerates the manifest (one entry fewer, exact against the live tree and shrink-only against `HEAD`) and corrects the v7.0.3 row's line count from 1594 to the verified 1593. |
| 7.0.3 | 2026-09-12 | **fix: a refused evolution restart now names and records its real reason.** An agent-requested restart whose reviewed commit was already in `HEAD` was refused with "the live checkout no longer matches the exact reviewed evolution commit" while a *concurrent* task's uncommitted work sat in the shared worktree — and the refusal wrote nothing durable, so the cycle stayed unabsorbed for ~10.5 hours with its cause reconstructible only from chat (the owner's own later Restart row, `reset_unsynced_rescued_then_reset dirty_count=16`, names what the agent path never reported). `ouroboros/server_restart.assess_evolution_restart_claim` is now the single owner of that verdict — receipt, authority, reviewed `HEAD`, unsynced worktree, read through `supervisor.git_ops` so the tree it classifies is the tree the restart would move — and every refusal leaves exactly one durable `evolution_restart_refused` row (`source="claim"` for the verdict, `source="downstream"` for the checkout/update gate's own refusal). A dirty worktree is refused rather than reset, because resetting would discard another actor's uncommitted work; the message names the owner's Restart, the consented path that rescues and resets. `server.py` leaves the size-ratchet debt (1628 → 1593 lines). |
| 7.0.2 | 2026-09-12 | **feat: the owner decides whether a route LIMIT may spend on the backup route.** `OUROBOROS_ROUTE_LIMIT_FALLBACK` (`allow` shipped / `deny`) answers one question the old code settled silently: when a provider route hits a limit, may the round walk the cross-model chain and dial another — possibly metered — route? `deny` refuses that substitution for the closed limit set (`provider_transient`, `rate_limit`, `subscription_window_exhausted`), so the round ends on its own typed reason; every other failure class keeps its decision under either value, the shipped default is byte-for-byte the previous behaviour, and the knob is one-directional (it can only deny a rotation, never enable one). A policy-caused denial is disclosed durably as `route_limit_policy_denied` rather than suppressing a spend decision silently. `tests/test_route_limit_policy.py` freezes the pre-change decision for every error kind and scans the producers so a new kind cannot enter the vocabulary unlisted — closing the class of regression in which a changed policy default silently removed a rotation that already worked while every test stayed green. |
| 7.0.1 | 2026-09-11 | **fix: an honest acceptance review is no longer discarded.** A task-acceptance reviewer that declares the objective `solved` while honestly marking a criterion `partial`/`missing`/`rejected` kept its verdict at the non-solved tiers only; at `solved` the same well-formed response was stamped `parse_status=malformed` and its vote dropped, so a unanimous honest gap collapsed the quorum and the owner was told "reviewers did not reach a valid quorum" — a false account of reviewers that had in fact agreed (traced from a fully-solved task whose three live reviewers all found the same real gap). The tier↔criteria coherence now lives where it belongs, in the release-clean bar (`task_acceptance_is_clean`), and a criterion-level gap costs the clean contribution exactly as `criteria_refs_unresolved` does — never the vote. `_criteria_shape_valid` is a shape predicate and no longer takes a tier. Quorum arithmetic, the FAIL veto and the clean bar are unchanged. |

...
[![Version 7.0.16](https://img.shields.io/badge/version-7.0.16-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v7.0.16/Ouroboros-7.0.16.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v7.0.16/Ouroboros-7.0.16-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v7.0.16/ouroboros_7.0.16_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.16/ouroboros-7.0.16-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.16/ouroboros-7.0.16-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.16/Ouroboros-7.0.16-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.16/Ouroboros-7.0.16-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 7.0.16 | 2026-09-14 | **fix: review retries honor structured rate-limit recovery facts.** A reviewer 429 previously classified as a generic transient and re-sent four seconds later even when the provider returned a structured reset timestamp, so triad/scope quorum degraded across the window. Now `classify_llm_exception` accepts only structured future metadata (RFC `Retry-After` delay-seconds or a numeric `X-RateLimit-Reset` epoch timestamp, below the deterministic millisecond threshold), keeps kind `provider_transient`, and attaches `reset_at`/`retry_after_sec` the same way the subscription-window class does. The one existing packet review retry consumes that classified value as a bounded (≤60 s) same-route wait, refuses a second paid send when the slot deadline cannot absorb it, and preserves the historical four-second backoff for absent, malformed, or past facts. A failed reviewer now preserves `reset_at` as typed failure evidence. Alternate reviewer routing and account-level RPM remain open work: they cannot be closed by per-process pacing. |
| 7.0.15 | 2026-09-14 | **refactor: restore coherent ownership around plan-bound work items.** The v7.0.14 closure mechanism is behavior-preserving: its exact plan-request fingerprint now lives beside canonical plan identity in `plan_spec.py`, runtime-data/settings evidence denial lives with evidence resolution in `plan_evidence.py`, and the one-caller `plan_review.py` passthrough is gone. A frozen digest regression prevents accidental migration to the different compact spec-hash wire. The plan header now names `work_item_refs`, the persistence map documents `commit_intent.work_item_binding` and restart-verified closure, and the architecture map names each actual owner. Work-item normalization, evolution validation, closed-wave binding, and transaction binding remain together; omitted versus explicit-empty semantics and absorb-only closure are unchanged. |
| 7.0.14 | 2026-09-14 | **fix: evolution absorption closes only plan-bound work items.** Improvement Backlog status previously depended on one legacy campaign-level nomination, so autonomous cycles could absorb reviewed fixes without marking the exact item resolved, while a stale campaign link could close the wrong one. `plan_task` now carries a bounded `spec.work_item_refs` list in plan identity and the reviewer packet; evolution validates `ibl-*` references against open backlog state, the current closed wave is copied into the commit intent, and the two restart-verified absorption paths close only that transaction's commit-bound refs. An omitted field preserves the legacy fallback, while an explicit empty list replaces it. |
| 7.0.13 | 2026-09-14 | **test: review retry pacing is fast and covers its pre-wait deadline.** The synthetic actor-truth timeout now replaces the real interruptible sleep while asserting that the retry requested the production backoff, so transport/parse/semantic truth no longer costs four wall-clock seconds. A direct actor regression also proves that a logical review window shorter than that delay refuses before sleeping or dispatching a second request, preserves the original provider exception, and records the separate deadline stop reason. Production retry behavior is unchanged. |
| 7.0.12 | 2026-09-14 | **fix: transient review retries wait before repeating the same physical request.** Packet API review rows already allowed one byte-identical second send after a terminal retryable exception, but issued it immediately, so a short provider rate-limit window commonly failed both attempts and collapsed triad/scope quorum. That existing resend now reuses the shared initial transient backoff and owner-interruptible sleep; durable cancellation or the slot logical deadline stops during the wait without another dispatch. Unknown/non-retryable outcomes remain terminal, while empty-output and task-acceptance format repair retain their immediate bounded resend. |
